...
def test_device_change(app, qtbot, mock_config):
    ...
    mock_config.set.assert_called_with("input_device_index", 1)
...
def test_set_model_list(app, qtbot, mock_config):
    ...
    mock_config.set.assert_called_with("formatter_model", "m2")
    
    # Test with empty text
    window.on_model_changed("")
    # No new call, so count should still be 1
